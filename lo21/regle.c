...
regle *supprimerPremisse(regle *r, char *texte) {
    ...
    if (r->premisse == NULL) {
        printf("Il n'y a pas de premisse pour le moment\n");
        return r;
    }
    ...
    if (p->next == NULL) {
        printf("La premisse donnee n'est pas dans la liste\n");
        return r;
    ...
    // Si on a trouvé : p->next est l'élément à supprimer
        ...
        free(tmp);
        printf("La premisse donnee a bien ete supprimee\n");
        return r;
    }
}
...
void afficherPremisse(regle *r) {
    if (r != NULL) {
        if (r->premisse == NULL) {
            ...
            printf("Premisse en tete : %s\n", r->premisse->proposition);
        }
    }else {
        printf("Erreur : regle inexistante.\n");
        return;
    }
}
